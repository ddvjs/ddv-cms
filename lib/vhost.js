...
const CacheTemp = require('../utils/cache-temp')
const cacheTemp = new CacheTemp()
...
function getNuxtRender (templateId, req, res) {
  console.log(111, cacheTemp.value)
  if (nuxts[templateId]) {
    var index = cacheTemp.value.indexOf(nuxts[templateId].options.__program)
    // 有更新文件
    if (nuxts[templateId].options.__program && index > -1) {
      return createNuxtRender(templateId, req, res)
        .then(nuxt => {
          if (typeof nuxts[templateId].close === 'function') {
            return nuxts[templateId].close()
              .then(() => {
                cacheTemp.remove(index)

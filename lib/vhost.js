...
function vhostRender (req, res, next) {
  ...
  getTemplateIdByHost(host, req, res)
    .then(templateId => {
      return getNuxtRender(templateId, req, res).then(render => {
...
function getNuxtRender (templateId, req, res) {
  if (nuxts[templateId]) {
    ...
    return createNuxtRender(templateId, req, res).then(nuxtRender => {
...
function createNuxtRender (templateId, req, res) {
  return getTemplateInfo(templateId, req, res)
    .then(info => {
      return getOptions(info.configFile).then(options => {
        ...
        fs.stat(distDir, (err, stats) => {
          ...
          if (isError) {
            if (
              ...
              vhostOptions
                .loadBuildFiles(options.rootDir, vhostOptions.processPool)
                .catch(e => {
                  e =
                    e ||
                    new Error(
